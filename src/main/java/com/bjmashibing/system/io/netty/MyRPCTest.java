...
/*
    ...
    3，动态代理呀，序列化，协议封装
    4，连接池
    5，RPC远程调用是什么？？就像调用本地方法一样去调用远程的方法，面向java中就是所谓的 面向interface开发
    服务端有接口的实现，客户端拿到接口，通过动态代理拿到代理类，
    客户端调用方法实际上就是通过代理类把数据传递给服务端，由服务端的实现方法真正去处理数据，结果返回
 */
...
public class MyRPCTest {
    ...
    public void startServer(){
        ...
        MyCar car = new MyCar(); //怎么最终调用到它呢??dubbo是怎么使用的??
        // 我觉得肯定是得到接口名之后,从某个集合中拿出实现类
        MyFly fly = new MyFly();
        //Dispatcher就相当于一个全局的集合
        Dispatcher dis = new Dispatcher();
        dis.register(Car.class.getName(),car); //<接口名, 实现类>
        dis.register(Fly.class.getName(),fly);

        NioEventLoopGroup boss = new NioEventLoopGroup(20);
        NioEventLoopGroup worker =  boss; //混杂模式
    ...
    public static <T>T proxyGet(Class<T>  interfaceInfo){
        ...
        return (T)Proxy.newProxyInstance(loader, methodInfo, new InvocationHandler() {
            ...
            public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                ...
                //2，requestID+message  ，本地要缓存 ,并发情况下，要知道返回给哪个线程
                //协议：【header<>】【msgBody】
...
//                CountDownLatch countDownLatch = new CountDownLatch(1);
                ...
                channelFuture.sync();  //io是双向的，你看似有个sync，她仅代表out 这只是out的future

//                countDownLatch.await();

                //5，？，如果从IO ，未来回来了，怎么将代码执行到这里
                //（睡眠/回调，如何让线程停下来？你还能让他继续。。。）
                //原来用countDownLatch,当前线程等待接收完成,现在不仅要等待接收完成,还要得到返回值
                //现在用CompletableFuture,主线程创建CompletableFuture,调用get阻塞,未来回调调用complete
                //但是msb多线程课中,CompletableFuture似乎是不用调用get的??
                return res.get();//阻塞的,所以就不用countDownLatch
            }
        });
    }
...
//        System.out.println("server handler :"+ requestPkg.content.getArgs()[0]);
        ...
        //关注rpc通信协议  来的时候flag 0x14141414 回的时候24，就像syn - syn+ack

        //有新的header+content
        String ioThreadName = Thread.currentThread().getName();
        //1,可以直接在当前线程既处理IO又处理业务(函数调用和返回),但是会阻塞其他IO处理

        //2，自己创建线程池,抛出线程处理业务
        //3,使用netty自己的eventloop（runAlltasks阶段）来处理业务及返回
        //eventloop又是一个executor
        ctx.executor().execute(new Runnable() {
//        ctx.executor().parent().next().execute(new Runnable() {
// 扔给其他eventloop,这么做有什么意义吗,有可能某个socket比较热，那可以把业务处理打散到其他线程上
//像redis IO Threads多线程，只负责到解码。命令具体的执行都达到单个线程,你可以自己开一个线程,也可以选择某一个eventloop
            @Override
            public void run() {
...
class ResponseMappingCallback {
    ...
    public static void runCallBack(Packmsg msg){
        CompletableFuture cf = mapping.get(msg.header.getRequestID()); //CompletableFuture, 为了把返回值返回
//        runnable.run(); //原来使用countdownlatch
        cf.complete(msg.getContent().getRes()); //解除get的阻塞
